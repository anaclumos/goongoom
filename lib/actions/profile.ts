...
import { DEFAULT_QUESTION_SECURITY_LEVEL, isQuestionSecurityLevel } from '@/lib/question-security'
import type { SocialLinks, UserProfile } from '@/lib/types'
import { fetchNaverBlogTitle, normalizeNaverBlogHandle } from '@/lib/utils/social-links'

export type ProfileActionResult<T = unknown> = { success: true; data: T } | { success: false; error: string }
...
export async function updateProfile(data: {
  ...
  return await withAudit({ action: 'updateProfile', payload: data }, async () => {
    ...
    try {
      ...
      if (data.questionSecurityLevel && !isQuestionSecurityLevel(data.questionSecurityLevel)) {
        return { success: false, error: t('invalidSecuritySetting') }
      }

      const validatedSecurityLevel =
        data.questionSecurityLevel && isQuestionSecurityLevel(data.questionSecurityLevel)
...
export async function fetchNaverBlogTitleAction(handle: string): Promise<ProfileActionResult<string>> {
  const t = await getTranslations('errors')
  try {
