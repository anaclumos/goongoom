...
export default async function UserProfilePage({ params, searchParams }: UserProfilePageProps) {
  ...
  const [dbUser, { answeredQuestions }, t, tCommon, , tAnswers, locale, tProfile, tSocial] = await Promise.all([
  ...
  const questionsWithAnswers = answeredQuestions
    .map((qa) => {
      ...
      const sender = !qa.isAnonymous && qa.senderClerkId ? senderMap.get(qa.senderClerkId) : null
      return {
        ...qa,
    ...
    <MainContent>
      <Card className="mb-6">
        <CardContent className="flex items-center gap-6">
          <Avatar className="size-24 ring-2 ring-primary/30" key={clerkUser.avatarUrl}>
            {clerkUser.avatarUrl && <AvatarImage alt={displayName} src={clerkUser.avatarUrl} />}
            <AvatarFallback>{displayName[0] || '?'}</AvatarFallback>
          </Avatar>
          <div className="flex flex-1 flex-col gap-1">
            <h1 className="font-semibold text-foreground text-xl">{displayName}</h1>
            <p className="text-muted-foreground text-sm">{clerkUser.username || username}</p>
