...
export const getFriendsAnswersPaginated = query({
  args: {
    clerkId: v.string(),
  ...
  handler: async (ctx, args) => {
    ...
    const paginatedAnswers = await ctx.db
      .query('answers')
      .filter((q) => q.eq(q.field('deletedAt'), undefined))
      .order('desc')
      .paginate(args.paginationOpts)

    const questionIds = [...new Set(paginatedAnswers.page.map((a) => a.questionId))]
    const questions = await Promise.all(questionIds.map((id) => ctx.db.get(id)))
    const validQuestions = questions.filter((q): q is NonNullable<typeof q> => q !== null && !q.deletedAt)
    const questionMap = new Map(validQuestions.map((q) => [q._id, q]))
    ...
    const filteredPage = paginatedAnswers.page
      .map((answer) => {
        const question = questionMap.get(answer.questionId)
        if (!question) {
          return null
